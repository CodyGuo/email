...
func (m *Message) Bytes() []byte {
    ...
    t := time.Date(2015, time.June, 1, 10, 30, 0, 0, time.Local)

    // t := time.Now()
    buf.WriteString("Date: " + t.Format(time.UnixDate) + "\n")
    ...
    if len(m.Attachments) > 0 {
        for _, attachment := range m.Attachments {
            ...
            // f := make([]byte, base64.StdEncoding.EncodedLen(len(attachment.Filename)))
            // base64.StdEncoding.Encode(f, []byte(attachment.Filename))
            // buf.Write(f)
